    ...
    public class ExpressionEvaluator : ExpressionEvalBase {
        ...
        protected override void ValidateProperty(string propertyName) {
            if (!_propDict.Contains(propertyName)) {
                ReportParseError(string.Format(CultureInfo.InvariantCulture, "Property '{0}' has not been set.", propertyName));
            }
        }

        protected override object EvaluateFunction(string functionName, ArrayList args) {
            MethodInfo methodInfo = TypeFactory.LookupCustomFunction(functionName);
            if (methodInfo == null) {
                return ReportParseError(String.Format("Unknown function: '{0}'", functionName));
            }
        ...
        protected override void ValidateFunction(string functionName, int argCount) {
            MethodInfo methodInfo = TypeFactory.LookupCustomFunction(functionName);
            if (methodInfo == null) {
                ReportParseError(String.Format("Unknown function: '{0}'", functionName));
            }
            if (methodInfo.GetParameters().Length != argCount) {
                ReportParseError(String.Format("Function '{0}' expects {1} arguments while {2} were supplied", functionName, methodInfo.GetParameters().Length, argCount));
            }
        }
