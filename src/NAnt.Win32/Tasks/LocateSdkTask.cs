...
namespace NAnt.Win32.Tasks {
	...
	internal class LocateSdkTask : Task {
		...
		protected override void ExecuteTask() {
			...
			for(int i = 0; i < installedWinSdkVersions.Length; i++) {
				...
				if (minSdkVersion <= loopSdkVersion) {
					...
					string[] netFxDirs = null;
					if (_minWinSdkVer == "v8.1A") {
						string installFolder = sdkRegSubKey.OpenSubKey(_minWinSdkVer).GetValue("InstallationFolder").ToString();
						netFxDirs = new string[] {
							installFolder,
							Path.Combine(installFolder, "bin"),
							Path.Combine(installFolder, @"bin\NETFX 4.5.1 Tools")
						};
					} else if (_minWinSdkVer == "v8.0A") {
						string installFolder = sdkRegSubKey.OpenSubKey(_minWinSdkVer).GetValue("InstallationFolder").ToString();
						netFxDirs = new string[] {
							installFolder,
							Path.Combine(installFolder, "bin"),
							Path.Combine(installFolder, @"bin\NETFX 4.0 Tools")
						};
					}
					else {
					...
					sdkFound = FindSDK(netFxDirs, maxNetVersion, minNetVersion);
		...
		private bool FindSDK(string[] netFxDirs, Version maxNetVersion, Version minNetVersion) {
			if (netFxDirs == null) return false;
